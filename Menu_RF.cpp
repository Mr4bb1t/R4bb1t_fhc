...
static uint16_t wfColor(uint8_t v) {
    ...
    int   idx = (int)pos;
    if (idx >= 8) return WF_PALETTE[8];
    return lerpColor(WF_PALETTE[idx], WF_PALETTE[idx + 1], pos - idx);
}

static int wfHalfWidth(uint8_t v) {
    return constrain(
        (int)map(v, 0, 255, WF_HALF_MIN, WF_HALF_MAX),
        WF_HALF_MIN, WF_HALF_MAX
    );
}
...
void displayRF_Analyser() {
    ...
    tft.setCursor(0,   axisY); tft.print("431");
    tft.setCursor(50,  axisY); tft.print("433.9");
    tft.setCursor(104, axisY); tft.print("436");
