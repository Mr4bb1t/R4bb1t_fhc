...
public:
  ...
  bool decodeNextFrame(uint16_t *displayBuf) {
    ...
    while (frameBytesDone < _frameSize) {
      ...
      size_t out_bytes =
          32768 - _dictWritePos; // MUST be the exact distance to wrap!
      ...
      tinfl_status status =
          tinfl_decompress(_inflator, _inBuf + _inBufPos, &in_bytes, _dictBuf,
                           _dictBuf + _dictWritePos, &out_bytes, flags);

      _inBufPos += in_bytes;
      _dictWritePos += out_bytes;
      if (_dictWritePos >= 32768)
        _dictWritePos = 0;
